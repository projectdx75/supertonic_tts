...
class UnicodeProcessor:
    ...
    def _preprocess_text(self, text: str, lang: Optional[str] = None) -> str:
        ...
        10. Wrap in language tags (Supertonic-2 requirement)

        Args:
            text: Raw input text
            lang: Optional language code (ko, en, zh, ja). If None, auto-detects.

        Returns:
            Preprocessed and normalized text with language tags
        """
        ...
        # Supertonic-2 Compatibility: Wrap text in language tags
        # Detect language if not provided (Simple heuristic: Hangul = ko, Default = en)
        if lang is None:
            # Check for Hangul Syllables (AC00-D7A3) or Jamo (1100-11FF, 3130-318F, A960-A97F, D7B0-D7FF)
            has_korean = any(
    ...
    def __call__(self, text_list: list[str], lang: Optional[str] = None) -> tuple[np.ndarray, np.ndarray]:
        """Process a list of texts into model inputs.

        Args:
            text_list: List of text strings to process
            lang: Optional language code (ko, en, zh, ja)
...
class Supertonic:
    ...
    def sample_noisy_latent(self, duration: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        # FIX: Set random seed for consistent voice style across chunks
        np.random.seed(42) 
    ...
    def __call__(
        self, text_list: list[str], style: Style, total_step: int = 5, speed: float = 1.05, lang: Optional[str] = None
